...
const BookingProducts = (props) => {
  ...
  useEffect(() => {
    const url = `https://whispering-bayou-15079.herokuapp.com/bookingClient?email=${
      user.email
  ...
  const handleDeleteItem = (id) => {
    fetch(`https://whispering-bayou-15079.herokuapp.com/bookingClient/${id}`, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then((data) => {
          ...
          <TableBody>
            {clients.map((client) => (
              <StyledTableRow key={client._id}>
                ...
                <StyledTableCell align="right">
                  <Button
                    onClick={() => handleDeleteItem(client._id)}
                    variant="contained"
                  >
                    Delete
                  </Button>
